...
class ClientsEmailsSQLController(SQLController):
    def __init__(self, session: Session, account_id: str):
        self.session = session
        self.account_id = account_id

    def upsert(self, data):
        clientsEmails: ClientsEmails = ClientsEmails.query.get(data['id'])
        if clientsEmails:
            self.update(clientsEmails, data)
        else:
            self.write(data)

    def update(self, clientsEmails, data):
        clientsEmails.client_pims_id = data['client']['pimsId']
        clientsEmails.is_primary = data['isPrimary']
        clientsEmails.email = data['email']

        self.session.add(clientsEmails)
        self.session.commit()

    def write(self, data):
        clientsEmails = ClientsEmails()
        clientsEmails.client_pims_id = data['client']['pimsId']
        clientsEmails.id = data['id']
        clientsEmails.is_primary = data['isPrimary']
        clientsEmails.email = data['email']
